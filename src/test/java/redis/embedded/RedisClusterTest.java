    ...
    try {
      pool =
          new JedisSentinelPool(
              "ourmaster",
              new HashSet<String>(Arrays.asList("localhost:26379", "localhost:26380")));
      jedis = testPool(pool);
    } finally {
      if (jedis != null) pool.returnResource(jedis);
      cluster.stop();
    ...
    try {
      pool1 =
          new JedisSentinelPool(
              master1,
              new HashSet<String>(
                  Arrays.asList("localhost:26379", "localhost:26380", "localhost:26381")));
      pool2 =
          new JedisSentinelPool(
              master2,
              new HashSet<String>(
      ...
      if (jedis1 != null) pool1.returnResource(jedis1);
      if (jedis2 != null) pool2.returnResource(jedis2);
      if (jedis3 != null) pool3.returnResource(jedis3);
      cluster.stop();
    }
  }
